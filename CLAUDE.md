...
- `src/lib/seo.ts`: helpers de JSON-LD (`websiteSchema`, `articleSchema`, `creativeWorkSchema`) con un `AUTHOR` (Person) compartido. `Layout.astro` acepta `image`/`type`/`jsonLd` opcionales — cuando no se pasa `image`, cae al isotipo como default de `og:image`/`twitter:image`. Home usa `WebSite`, posts del blog usan `BlogPosting`, portfolio usa `CreativeWork`.
...
- Pendiente real: no hay una imagen OG de marca propiamente dicha (1200×630) — el fallback es el isotipo (500×500, circular, con harto espacio transparente alrededor), que funciona pero no es un banner social pensado para eso. Ver ROADMAP.md si en algún momento vale la pena diseñar uno.
